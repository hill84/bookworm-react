...
const NoteForm: FC<NoteFormProps> = ({
  ...
  const onChangeSelect = (name: string) => (e: ChangeEvent<{ name?: string; value: unknown }>): void => {
    e.persist();
  ...
  const onSubmit = (e: FormEvent): void => {
    ...
    if (changes) {
      ...
      if (Object.keys(errors).length === 0) {
        ...
        const newNoteRef: DocumentReference<DocumentData> = notesRef(uid).doc();
        const ref: DocumentReference<DocumentData> = nid ? noteRef(uid, nid) : newNoteRef;
