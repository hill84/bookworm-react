...
import React, { FC, forwardRef, ReactElement, Ref, useCallback, useContext, useEffect, useMemo, useState } from 'react';
...
const Review: FC<ReviewProps> = ({
  ...
  const onFlag = useCallback((value?: string) => {
    if (user) {
      const flag = {
        value,
  ...
  const onToggleCommentsPanel = () => setSelectedRid(s => s === review.createdByUid ? '' : review.createdByUid);

  const onEditComment = () => setIsEditingComment(true);

  const onCancelComment = () => setIsEditingComment(false);

  // const onCloseCommentsPanel = () => setSelectedRid('');
  ...
  const classNames = useMemo((): string => `${isOwner ? 'own review' : 'review'} ${review.flag ? `flagged ${review.flag.value}` : ''}`, [isOwner, review]);
  const selected = useMemo((): boolean => Boolean(selectedRid) && selectedRid === review.createdByUid, [review.createdByUid, selectedRid]);

  return (
    <>
      <div className={classNames} id={review.createdByUid}>
        <div className='row'>
